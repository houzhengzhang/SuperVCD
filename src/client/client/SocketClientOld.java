...
public class SocketClientOld {
    ...
    private static SocketClientOld socketClient = null;
     ...
     * @return
     */
    public static SocketClientOld getSocketClient() {
        try {
            if (socketClient == null) {
                socketClient = new SocketClientOld();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
        SocketClientOld sc = SocketClientOld.getSocketClient();
        for (int i = 0; i < 5; i++) {
            // 向服务端发送数据
